...
type Inspection = { id: string; inspection: { fields: Array<{ path: string; name: string; type: string; nullable: boolean; sample_count: number; missing_rate: number; sample_values: string[] }>; mappings: Array<{ source_path: string; target_field: string | null }> } };
...
export default function ProjectOperations({ language, projectId, projectDirectory, outputs, fields, activeSpecificationVersion = 1, projectNonConfidential = true, onSpecificationActivated = () => undefined }: Props) {
  ...
  const refreshMemory = () => run(async () => { const value = await request(`/api/projects/${projectId}/memory`) as MemoryEntry[]; setMemory(value); return value; });
  const forgetMemory = (entry: MemoryEntry) => run(async () => {
    await request(`/api/projects/${projectId}/memory/${encodeURIComponent(entry.kind)}/${encodeURIComponent(entry.key)}`, { method: "DELETE" });
    setMemory((current) => current.filter((item) => item.kind !== entry.kind || item.key !== entry.key));
    return t.forgetMemory;
  });
  ...
  const formatDuration = (seconds: number | null) => seconds === null ? t.noValue : `${numberFormat.format(seconds)} s`;
  const isStaleSource = (runRecord: RunHistoryRecord, previousRun: RunHistoryRecord | undefined) => {
    if (!previousRun) return false;
    if (!runRecord.freshness_at) return Boolean(previousRun.freshness_at);
    if (!previousRun.freshness_at) return false;
    const current = Date.parse(runRecord.freshness_at);
    const previous = Date.parse(previousRun.freshness_at);
    return Number.isFinite(current) && Number.isFinite(previous) && current <= previous;
  };
  const renderHistory = () => history && (history.length === 0 ? <p>{t.noRuns}</p> : <div className="run-history" role="list">{history.map((runRecord, index) => {
    const previousRun = history[index + 1];
    const metricKeys = [...new Set([...Object.keys(runRecord.metrics), ...(previousRun ? Object.keys(previousRun.metrics) : [])])].sort();
    const stale = isStaleSource(runRecord, previousRun);
    return <article className="run-history-row" key={runRecord.id} role="listitem"><header><strong>{runRecord.status}</strong>{stale && <span className="status revision">{t.staleSource}</span>}<span>{t.duration}: {formatDuration(runRecord.duration_seconds)}</span><time dateTime={runRecord.started_at}>{dateFormat.format(new Date(runRecord.started_at))}</time></header>{metricKeys.length === 0 ? <p>{t.noMetrics}</p> : <ul>{metricKeys.map((key) => {
      const hasCurrentValue = Object.prototype.hasOwnProperty.call(runRecord.metrics, key);
      const hasPreviousValue = previousRun ? Object.prototype.hasOwnProperty.call(previousRun.metrics, key) : false;
      const currentValue = hasCurrentValue ? runRecord.metrics[key] : null;
      const previousValue = hasPreviousValue ? previousRun?.metrics[key] : null;
      const changed = typeof currentValue === "number" && typeof previousValue === "number" && currentValue !== previousValue;
      const delta = changed ? currentValue - previousValue : null;
      return <li key={key}><strong>{key}</strong>: {currentValue === null ? t.noValue : numberFormat.format(currentValue)}{delta !== null && <small> ({delta >= 0 ? "+" : ""}{numberFormat.format(delta)} {t.vsPrevious})</small>}</li>;
    })}</ul>}</article>;
  })}</div>);
  ...
  return <section className="operations" aria-labelledby="operations-title"><h2 id="operations-title">{t.title}</h2><p>{t.formats}</p>
    ...
    <details><summary>{t.schedule}</summary><label><input type="checkbox" checked={nonConfidential} onChange={(event) => setNonConfidential(event.target.checked)} /> {t.safe}</label><button disabled={busy || !destination} onClick={previewSchedule}>{t.preview}</button><button disabled={busy || !nonConfidential} onClick={activateSchedule}>{t.activate}</button><button disabled={busy || !scheduleId} onClick={() => run(async () => { const value = await request(`/api/schedules/${scheduleId}/runs`) as RunHistoryRecord[]; setHistory(value); return value; })}>{t.history}</button>{renderHistory()}</details>
    ...
    <details onToggle={(event) => { if (event.currentTarget.open) void refreshMemory(); }}><summary>{t.memory}</summary>{memory.length === 0 ? <p>{t.emptyMemory}</p> : <div className="table-wrap"><table><thead><tr><th>{t.memoryKind}</th><th>{t.memoryKey}</th><th>{t.memoryValue}</th><th /></tr></thead><tbody>{memory.map((entry) => <tr key={`${entry.kind}:${entry.key}`}><td>{entry.kind}</td><td>{entry.key}</td><td>{typeof entry.value === "string" ? entry.value : String(entry.value)}</td><td><button disabled={busy} onClick={() => void forgetMemory(entry)}>{t.forgetMemory}</button></td></tr>)}</tbody></table></div>}</details>
