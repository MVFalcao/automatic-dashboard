...
export default function ProjectOperations({ language, projectId, projectDirectory, outputs, fields }: Props) {
  ...
  const [codexOAuth, setCodexOAuth] = useState<CodexOAuthStatus | null>(null);
  ...
  const cancelCodex = () => run(async () => {
    if (!codexOAuth) return null;
    await request(`/api/providers/oauth/codex/${codexOAuth.session_id}`, { method: "DELETE" });
    const cancelled = { ...codexOAuth, status: "cancelled" };
    setCodexOAuth(cancelled);
    return cancelled;
  });
